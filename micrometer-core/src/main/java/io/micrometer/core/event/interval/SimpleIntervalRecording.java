...
package io.micrometer.core.event.interval;
...
public class SimpleIntervalRecording<T> implements IntervalRecording<T> {
	...
	public IntervalRecording<T> error(Throwable error) {
		...
		verifyIfHasNotStopped();
		if (this.error != null) {
            LOGGER.trace("Only one error can be attached");
			return this;
		}
	...
	private void verifyIfHasStarted() {
		if (this.started == 0) {
            LOGGER.trace("IntervalRecording hasn't been started");
		}
	}

	private void verifyIfHasNotStopped() {
		if (this.stopped != 0) {
            LOGGER.trace("IntervalRecording hasn't been stopped");
		}
	}
